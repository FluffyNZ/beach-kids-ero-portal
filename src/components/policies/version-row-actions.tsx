...
import { getSignedUrlForPolicy } from "@/lib/actions/policies";

export function VersionRowActions({ storagePath }: { storagePath: string }) {
  async function openFile(download: boolean) {
    const url = await getSignedUrlForPolicy(storagePath, download);
